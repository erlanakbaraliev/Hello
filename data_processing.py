"""Data loading and preprocessing for air-quality forecasting."""
...
def validate_and_prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_columns = POLLUTANT_COLUMNS + ["latitude", "longitude"]
...
def combine_sources(uploaded_df: pd.DataFrame | None, live_df: pd.DataFrame | None) -> pd.DataFrame:
...
def split_train_test(df: pd.DataFrame, test_days: int = 7) -> tuple[pd.DataFrame, pd.DataFrame]:
    if len(df) <= test_days:
...
def seasonal_aggregates(df: pd.DataFrame, columns: list[str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    work = df.copy()
