...
def render_sidebar_branding() -> None:
    st.sidebar.page_link("Home.py", label="Urban Air Quality", icon="🌿")
    st.sidebar.divider()


def render_sidebar_workspace_nav() -> None:
    if current_user_id() is None:
        st.sidebar.caption("Sign in to access the workspace.")
        return
    st.sidebar.page_link("pages/1_Data_Explorer.py", label="Data Explorer")
    st.sidebar.page_link("pages/3_Dashboard.py", label="Dashboard")
    st.sidebar.page_link("pages/2_Predict_future_quality.py", label="Predict")
    st.sidebar.page_link("pages/4_History.py", label="History")
    st.sidebar.page_link("pages/6_HelpDesk.py", label="HelpDesk")
...
def render_signed_in_sidebar_account() -> None:
    ...
    st.sidebar.divider()
    st.sidebar.caption(f"Signed in as **{html.escape(username)}**")
    st.sidebar.button(
        "Log out",
